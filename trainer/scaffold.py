...
class SCAFFOLDImageNet(BaseTrainer):
    """
    ...
    def __init__(self, params):
        ...
        for i in range(self.num_clients):
            self.local_all_c.append(copy.deepcopy(self.global_c))
    ...
    def run(self):
        ...
        for federated_round in range(self.start_round, self.total_round + 1):
            ...
            if global_delta_c is None:
                global_delta_c = OrderedDict()
                for name, data in self.global_model.state_dict().items():
                    if 'num_batches_tracked' in name:
                        continue
                    global_delta_c[name] = torch.zeros_like(data)
            else:
                for name, data in global_delta_c.items():
                    data.data.zero_()
            ...
            #     self.global_c[name] = torch.zeros_like(param)
            # for i in range(self.num_clients):
            #     self.local_all_c[i] = copy.deepcopy(self.global_c)
            ...
            # for client_id in self.selected_clients:
                ...
                net_para = model.state_dict()
                local_c = self.local_all_c[client_id]
                c_new_para = copy.deepcopy(local_c)
                for key in local_c:
                    c_new_para[key] = local_c[key] - self.global_c[key] + (
                                global_model_para[key] - net_para[key]) / (tau * self.lr)
                    global_delta_c[key].add_((c_new_para[key] - local_c[key])/len(self.selected_clients))

                for name, data in self.local_all_c[client_id].items():
                    data.data.copy_(c_new_para[name])
